...
description: >
  验证 Python 项目的导入路径规范性，检测重复导入、循环导入、__init__.py 导出一致性等问题。
  支持重复导入检测、循环导入检测、包导出一致性验证、多格式报告生成。
  当需要代码审查、重构指导、质量门禁、模块结构优化、技术债务评估时使用。
  触发关键字：导入路径、导入检查、循环导入、重复导入、__init__.py 一致性、模块依赖。
---
