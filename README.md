...
- [🚀 快速开始](#-快速开始)
  - [环境要求](#环境要求)
  - [安装步骤（5 分钟完成）](#安装步骤5-分钟完成)
    - [1. 克隆或下载项目](#1-克隆或下载项目)
    - [2. 创建虚拟环境（推荐）](#2-创建虚拟环境推荐)
    - [3. 安装依赖](#3-安装依赖)
    - [4. 配置 API（⚠️ 重要）](#4-配置-api-重要)
    - [5. 启动程序](#5-启动程序)
  - [✅ 验证安装](#-验证安装)
- [📖 完整文档导航](#-完整文档导航)
  - [🌟 **新手必读**（从零开始）](#-新手必读从零开始)
  - [🔧 开发指南（适合开发者）](#-开发指南适合开发者)
  - [📑 API 文档（适合高级用户）](#-api-文档适合高级用户)
  - [🗂️ 模块文档](#-模块文档)
- [🎯 新架构核心优势](#-新架构核心优势)
  - [1. **六层分层架构** - 清晰的职责划分](#1-六层分层架构-清晰的职责划分)
  - [2. **依赖注入容器** - 自动管理所有组件](#2-依赖注入容器-自动管理所有组件)
  - [3. **外观模式** - 简化的 API 调用](#3-外观模式-简化的-api-调用)
  - [4. **LRU 缓存优化** - 性能提升 15 倍](#4-lru-缓存优化-性能提升-15-倍)
  - [5. **批量操作优化** - 降低调用开销](#5-批量操作优化-降低调用开销)
  - [6. **代码精简** - 从 5000 行降至 2000 行](#6-代码精简-从-5000-行降至-2000-行)
- [💡 快速使用指南](#-快速使用指南)
...
- [🔧 高级功能](#-高级功能)
  ...
  - [2. 翻译历史查询](#2-翻译历史查询)
  - [3. 撤销/重做操作](#3-撤销重做操作)
  - [4. 使用依赖注入容器](#4-使用依赖注入容器)
  - [5. 性能监控](#5-性能监控)
  - [4. 模型配置拆分（NEW!）](#4-模型配置拆分new)
- [🧪 测试](#-测试)
  - [运行测试](#运行测试)
  - [测试覆盖](#测试覆盖)
- [📊 性能指标](#-性能指标)
- [❓ 常见问题](#-常见问题)
  - [Q1: 启动时提示"未找到 API Key"](#q1-启动时提示未找到-api-key)
  - [Q2: 翻译速度慢怎么办？](#q2-翻译速度慢怎么办)
  - [Q3: 如何添加新的目标语言？](#q3-如何添加新的目标语言)
  - [Q4: 输出表格的格式是什么样的？](#q4-输出表格的格式是什么样的)
  - [Q5: 术语库如何备份？](#q5-术语库如何备份)
- [🤝 贡献](#-贡献)
  - [开发环境设置](#开发环境设置)
  - [代码规范](#代码规范)
- [📝 更新日志](#-更新日志)
